...
//        String[] resCoordinates = new String[100];
        ...
        Mat srcNone = Imgcodecs.imread(path);
...
//          Imgproc.drawMarker(src, centerList.get(i), color);
        ...
        Imgcodecs.imwrite("src/images/src_process_none"+index+".jpg", src);
...
//            System.out.println(centerList.get(i));
            ...
            String centerX = String.format("%.2f", centerList.get(i).x);
            String centerY = String.format("%.2f", centerList.get(i).y);

            FormatDouble fd = new FormatDouble();

            Coordinate coordinate = new Coordinate(fd.formatDouble(Double.parseDouble(centerX)), fd.formatDouble(Double.parseDouble(centerY)));
            coordinateList.add(coordinate);


            // 绘制坐标序号
            Imgproc.putText(src, strNum, centerList.get(i), 1, 1, color2, 1);
